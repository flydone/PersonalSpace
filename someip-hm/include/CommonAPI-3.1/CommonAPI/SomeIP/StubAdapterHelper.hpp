...
protected:
    bool findDispatcherAndHandle(const Message &message, const method_id_t &methodId) {
      (void) message;
      (void) methodId;
	  if (message.isRequestType()){	// walker.wang for  SOMEIPSRV_RPC_05: Fire & forget requests shall return no error
 ...
 protected:
    ...
    bool findDispatcherAndHandle(const Message &message, const method_id_t &methodId) {
        ...
        if (stub_ && foundInterfaceMemberHandler) {
			...
			// walker.wang for SOMEIPSRV_RPC_08:Do not reply to messages already carrying an error begin
			uint8_t retcode = (uint8_t)message.getReturnCode();
			retcode = retcode & 0x3F;	// SOMEIPSRV_RPC_07: Ignore the two most significant bits from return code
			if(retcode != 0){
				return false;
			}
			// walker.wang for SOMEIPSRV_RPC_08 end
...
private:
    ...
    inline bool dispatchMessageHelper(const Message &_message,
		...
		if(0 != client->ErrorCode) // walker.wang for send error to client
		{
			auto error = _message.createErrorResponseMessage((return_code_e)client->ErrorCode);
			 _connection->sendMessage(error);
		}
